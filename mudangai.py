...
def generate_stock_position_csv(
    ...
    for index, row in merged_data.iterrows():
        if row["LongYdAvailable"] < row["voll"]:
            merged_data.at[index, "policy"] = "build"
            build_vol = row["voll"] - row["LongYdAvailable"]
            T0_vol = row["LongYdAvailable"]
            merged_data.at[index, "long_vol"] = build_vol
            merged_data.at[index, "tod_long_vol"] = 0
            merged_data.at[index, "yd_long_vol"] = 0
        elif row["LongYdAvailable"] > row["voll"]:
            merged_data.at[index, "policy"] = "clear"
            clear_vol = row["LongYdAvailable"] - row["voll"]
            T0_vol = row["voll"]
            merged_data.at[index, "long_vol"] = clear_vol
            merged_data.at[index, "tod_long_vol"] = 0
            merged_data.at[index, "yd_long_vol"] = clear_vol
        else:
            merged_data.at[index, "policy"] = "T0"
            T0_vol = row["LongYdAvailable"]
            merged_data.at[index, "long_vol"] = T0_vol
            merged_data.at[index, "tod_long_vol"] = T0_vol  # 恢复原来的逻辑
            merged_data.at[index, "yd_long_vol"] = T0_vol

        merged_data.at[index, "T0_vol"] = T0_vol

        # # 根据策略更新 long_vol, tod_long_vol, yd_long_vol
        # if policy == "build":
        #     merged_data.at[index, "long_vol"] = build_vol
        #     merged_data.at[index, "tod_long_vol"] = 0
        #     merged_data.at[index, "yd_long_vol"] = 0
        ...
        #     merged_data.at[index, "long_vol"] = T0_vol
        #     merged_data.at[index, "tod_long_vol"] = T0_vol  # 恢复原来的逻辑
        #     merged_data.at[index, "yd_long_vol"] = T0_vol   # 恢复原来的逻辑

    # 生成母单数据
    def create_mother_order(order_data, trading_time, policy):
        ...
        order.iloc[:, 2] = order.iloc[:, 4].astype(str) + f"{policy}_{current_time_str}_{name}{policy.capitalize()}"
        ...
        order["max_pos_ratio"] = 0.3 if policy == "T0" else 0.1
        ...
        print(f"Close * long_vol 的总和 ({policy}):", total_sum)

        return order
    
    t0_order = create_mother_order(merged_data, trading_time_t0, "T0")
    twap_order = create_mother_order(merged_data, trading_time_twap, "TWAP")
