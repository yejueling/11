...
def generate_stock_position_csv(name, amount, date, stock_index, trading_time, policy, auction_vol, debt_available_vol, return_vol, is_t0_strict, can_limit_open):
    """
    ...
    # 计算交易量
    mer['amount'] = mer['Weight'] * amount
    mer['vol'] = mer['amount'] / mer['Close'] / 100
    ...
    for index, row in mer.iterrows():
        if row['LongYdAvailable'] < row['voll']:
            ...
            policy = 'T0'  # 可以添加一个默认策略
            T0_vol = row['LongYdAvailable']

        # 可以把结果存储在新的列中
        # mer.at[index, 'Policy'] = policy
    ...
    new_mer['trading_time'] = trading_time  
    new_mer['policy'] = 'T0'  
    ...
    # 计算第五列（amount）乘以第八列（tod_long_vol）的总和
...
if __name__ == '__main__':
    generate_stock_position_csv(name = 'Csi500', 
                                ...
                                trading_time = '[09:30:00,10:30:00]', 
                                policy='', 
                                auction_vol = 0, 
